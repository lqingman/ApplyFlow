# ApplyProof Devpost Submission Copy

This file contains ready-to-paste submission copy. Replace the bracketed placeholders before final submission.

## Elevator pitch

ApplyProof is an evidence-grounded job application copilot that helps candidates move faster without inventing who they are.
...
ApplyProof starts from a simple principle: **evidence before eloquence**. Known facts should be filled deterministically, generated writing should stay grounded in candidate evidence, and the applicant—not the agent—should make the final decision.

## What it does

ApplyProof is a Chrome Manifest V3 extension with a compact side panel and page-native review experience. An applicant can create an editable local profile, seed it with fictional Maya Chen demo data, or import a Word or text-based PDF resume. The original resume stays in extension-owned IndexedDB, and extracted values remain editable before saving.

After the user clicks **Scan & Autofill**, ApplyProof:
...
Cover letters use bounded job-description context plus selected resume evidence. If ApplyProof cannot safely find the job description, it asks the user to paste it rather than generating without context. Continue, Next, legal consent, and Submit always remain manual actions.
...
## What's next for ApplyProof

Next steps are deliberate, platform-by-platform compatibility validation; clearer local controls for saved profile and preference data; provider observability and cost controls; and a Chrome Web Store-ready privacy and security review. ApplyProof will continue to prioritize a small number of reliable integrations over a claim of universal autofill.
...
- Source code: https://github.com/lqingman/ApplyProof
- Demo video: [ADD PUBLIC YOUTUBE URL]

## Additional information: judge testing instructions

ApplyProof is a Chrome Manifest V3 extension. The supported judging path is the local Northstar Labs demo on macOS, Windows, or Linux with Chrome 116+, Node.js 20+, npm 10+, and Python 3.12+.

1. Clone https://github.com/lqingman/ApplyProof and follow the README setup steps.
...
5. Run `npm run build --workspace @applyproof/extension`.
6. Open `chrome://extensions`, enable Developer mode, choose **Load unpacked**, and select `apps/extension/dist`.
7. Open `http://localhost:5173`, click the ApplyProof toolbar icon, and choose **Load Maya demo data**.
8. Click **Scan & Autofill**. Review the deterministic fields and the grounded answers inserted into the page.
9. Focus an open-ended field, add an optional instruction, and regenerate it. Confirm existing values are preserved and ApplyProof never clicks **Submit application**.
