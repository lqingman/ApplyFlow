...
def test_rephrased_experience_description_is_replaced_with_resume_text(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    body = extraction_body()
    body["text"] = (
        f"{body['text']}\nExample Labs\nSoftware Developer\nMay 2024 - Present\n"
        "Built payment workflows in TypeScript.\nReduced checkout errors by 20%."
    )
    ...
    class RephrasingProvider:
        def extract_resume(self, request: ResumeExtractionRequest) -> ResumeExtraction:
            candidate = request.baseline.model_copy(deep=True)
            candidate.experience[0].description = (
                "Improved TypeScript payments and made checkout more reliable."
            )
            return candidate

    monkeypatch.setattr(main_module, "configured_provider", lambda: RephrasingProvider())

    response = post_extraction(body)

    assert response.status_code == 200
    assert response.json()["experience"][0]["description"] == (
        "Built payment workflows in TypeScript.\nReduced checkout errors by 20%."
    )
    assert "verbatim resume text" in response.json()["notes"][-1]
